...
const NoEntregados = () => {
  ...
  const [idNoEntregado, setIdNoEntregado] = useState("");
  ...
  async function acceptOrCancelEntrega(idEntrega, decision) {
    const data = {
      id: idEntrega,
      decision: decision,
    };
    console.log(data);
    const response = await fetch(`${url}/api/entregas_decision`, {
      method: "POST",
      headers: {
  ...
  return (
    <div className="p-2 w-full  fill-gray-400 ">
                ...
                <td className="flex gap-6 mt-1">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setIdNoEntregado(row.original.id);
                      setDiasPrestamo(row.original.dias_prestamo);
                      setAcceptModal(true);
                    }}
                  >
      ...
      <Modal isVisible={acceptModal} onClose={() => setAcceptModal(false)}>
        <div className=" p-6">
          <h3 className="text-xl font-semibold">Registrar Entrega</h3>
          <div className="mt-2 flex flex-col gap-1">
            ...
            <p claseName="text-base">
              Fecha de entrega:{" "}
              {String(date.getDate(date.setDate(date.getDate()))).padStart(
                2,
                "0"
              )}
            ...
            <p claseName="text-base">
              Fecha de devolución:{" "}
              {String(
                date.getDate(date.setDate(date.getDate() + diasPrestamo))
              ).padStart(2, "0")}
              -{String(date.getMonth() + 1).padStart(2, "0")}-
              {date.getFullYear()}
            </p>
          ...
          <form
            onSubmit={(e) => {
              ...
              // setDiasPrestamo(row.original.dias_prestamo);
              setIdNoEntregado("");
              acceptOrCancelEntrega(idNoEntregado, true);
      ...
      <Modal isVisible={cancelModal} onClose={() => setCancelModal(false)}>
        <div className=" p-6">
          <h3 className="text-xl font-semibold">Cancelar Entrega</h3>
          <div className="mt-2 flex flex-col gap-1">
            <h4 className="text-base font-semibold">Datos:</h4>
            <p claseName="text-base">Código del Libro: {libroCodigo}</p>
            <p claseName="text-base">Libro: {NombreLibro}</p>
            <p claseName="text-base">Pedido por: {nombreUser}</p>
            <p claseName="text-base">DNI: {dniUser}</p>
            <p claseName="text-base">Días de préstamo: {diasPrestamo} días</p>
            <p claseName="text-base">
              Fecha de entrega:{" "}
              {String(date.getDate(date.setDate(date.getDate()))).padStart(
                2,
          ...
          <form
            onSubmit={(e) => {
              ...
              // setDiasPrestamo(row.original.dias_prestamo);
              setIdNoEntregado("");
              acceptOrCancelEntrega(idNoEntregado, false);
              setAcceptModal(false);
            }}
            action=""
            method="post"
          ...
          >
            ...
            <button
              type="submit"
              className="bg-red-500 text-white p-2 hover:bg-green-400 rounded-md"
            >
              Eliminar
            </button>
          </form>
...
export const noEntregadosLoader = async () => {
  // await fakeNetwork();
